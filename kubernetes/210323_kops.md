...
VALIDATION ERRORS
KIND	NAME									MESSAGE
Pod	kube-system/kube-controller-manager-ip-172-XX.XX.XX.ec2.internal	system-cluster-critical pod "kube-controller-manager-ip-172-XX.XX.XX.ec2.internal" is not ready (kube-controller-manager)
Pod	kube-system/kube-scheduler-ip-172-XX.XX.XX.ec2.internal		system-cluster-critical pod "kube-scheduler-ip-172-XX.XX.XX.ec2.internal" is not ready (kube-scheduler)
...
`kops export kubecfg --admin` 명령어 입력 후  `kops validate cluster` 실행하면 오류 해결~! 근데 아직 validation failed 에러는 해결 안됨
