...
top-secret-bucket-trail에서 cloudwatch와 연결하기 위해 CloudWatch Logs를 편집
...
PutObject나 GetObject 필터링에 해당하는 로그를 AccessMetric (라벨을 부여)

그러한 개별 필터에 해당하는 로그들을 그룹핑해서 LogMetrics
...
CloudWatch Logs로 이동하여 람다 함수의 로그를 확인
...
함수의 실행 결과를 파일(OUTPUT.log) 로 저장하는 명령어 
- `cat OUPUT.log` 를 통해 파일의 내용을 확인하면 반환하는 값이 없기 때문에 아무 내용이 없다.
